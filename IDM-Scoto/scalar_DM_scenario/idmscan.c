...
//  double x0[10], R0[10], x1[10], R1[10], oldLike, newLike;                              
  int i0, i1, npoints=0, igood=0, iLLP=0, iHSCP=0, iWarn=0, iExcl=0;
...
//----- random scan over mH0, mA0, mHc-mA0 (lam2,lamL fixed)   
  mH0 = Uniform(100,500);  
  //deltaM = Normal(0.140,0.002); if (deltaM<0.135 || deltaM>0.145) continue;
  //deltaM = exp(Uniform(logdMmin,logdMmax));
  //deltaM = 0.1394+0.0004*Uniform(0,1);  
  deltaM = Normal(0.1397,0.0002); if (deltaM<0.139 || deltaM>0.142) continue;
  mHc = mH0+deltaM;  
...
//-------------------------------------------------------------------
  ...
  printf("\n---------------------------------------------------------------------------------\n");
  printf("%d) random point: %8.3f %8.3f %8.3f %8.5f\n", i0, mH0, mA0, mHc, deltaM);
...
#include "../include/SMODELS.inc" // SLHA interface with SModels:   smodels.slha  ==>  smodels.slha.smodelsslha
   ...
   {  printf(" highest r-value = %.2E",Rvalue); 
      if(Rvalue>0) {
        ...
        //printf("\n chi^2 = %.2E, -2logLike = %.2E ",SmoChi2, -2*log(SmoLike));
      }  
      if(status==1) { printf("\n excluded"); iExcl++; }
  ...
  sprintf(command,"cp smodels.slha ./scanpoints/idm-batch9-pt-%d.slha",igood);
  system(command);

  if(status>=0)
    fprintf(fgood, "%9.4f %9.4f %9.4f  %.2e  %.2e  %4.2f  %.2e  %2d  %.2e  %s  '%s'\n", 
