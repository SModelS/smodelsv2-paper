...
#ifdef DECAYS
{
   ...
   FILE* out=NULL;
   char txt[50];
   sprintf(filename, "pionDecayScan.csv");
   out = fopen(filename, "w");
   fprintf(out, "deltaM;channel;partialWidth;fpi\n");
   ...
   //   width=pWidth(pname,&LZ);
   //    printf("\n%s :   total width=%.3E \n and Branchings:\n",pname,width);
   //    printPartialWidth(width,LZ,stdout);
      printf("Scan over dm, ~H+ decays with pion switch");
      for (double dm = 0.1; dm < 10.; dm += 0.05){
        assignVal("MHC", pMass("~X") + dm);
        // only fermion decay
        assignVal("fpi", 0.);
        cleanDecayTable();
        width=pWidth(pname,&LZ);
        ...
        sprintf(txt, "total");
        fprintf(out,"%.4E;%s;%.4E;%d\n", dm, txt, width, -1);
        // thPi = pow(findValW("EE")/findValW("SW"),4)*pow(findValW("fpi"),2);
        // thPi /= 64*M_PI*pow(findValW("MW"),4);
        // thPi *= dm*dm*sqrt(dm*dm - pMass("pi+")*pMass("pi+"));
        // moPi = findBr(LZ,"pi+, ~X")*width;
        // ctau = 1./moPi*6.582E-25*3E8;
      }
      printPartialWidth(width,LZ,stdout);
      fclose(out);
