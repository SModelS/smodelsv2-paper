...
! open (status=0) or close (status=1) all input and output files
  ...
  ! ------------------------------
  ! open (status=0) or close (status=1) all input and output files
    subroutine MYPROSPINO_OPEN_CLOSE(status,filename)
      ...
      if (status == 0 ) then
         open(unit=nin2,  file=filename, action="read" )                         ! open all files here
         open(unit=ndat1, file=TRIM(filename)//".dat", action="write")
         open(unit=ndat2, file=TRIM(filename)//".dat2", action="write")
         open(unit=ndat3, file=TRIM(filename)//".dat3", action="write")
      else if (status == 1) then
         if ((final_state=='hh').or.(final_state=='ht')) then
         write(ndat1, fmt=form1)                                                                 &
         "    i1 i2  dummy0 dummy1 scafac  m1    m2      angle LO[pb]   rel_error NLO[pb]   rel_error   K    NLO_SUSY[pb]  "
    ...
    end subroutine MYPROSPINO_OPEN_CLOSE
